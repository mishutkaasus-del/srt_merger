...
def clean_parenthetical(match):
    inner = match.group(1)
    if has_cyrillic(inner): return ""
    if re.fullmatch(r"[\d\s]*", inner): return ""
    if re.match(r"(?i)asset\s*id", inner): return ""
    return f"({inner})"


def clean_line(line):
    if re.match(r"(?i)asset\s*id\s*[:\-]?\s*\d*", line.strip()): return ""
    ...
    for line in content.splitlines():
        ...
        if stripped == "":
            if current_lines:
                ...
                current_lines = []
            continue
        if re.fullmatch(r"\d+", stripped): continue
        if has_cyrillic(stripped): continue
    ...
    for e in entries:
        ...
        for i, (tok, is_dash) in enumerate(zip(tokens, token_is_dash)):
            ...
            if is_dash:
                ...
                if i > 0: imprecise_start.add(idx)
                if i < n - 1: imprecise_end.add(idx)
            all_words.append({"word": tok, "start": ms_to_ts(word_start_ms), "end": ms_to_ts(word_end_ms)})
...
def full_match(srt_norm, sub_words, search_from, total):
    for i in range(search_from, total):
        if srt_norm[i] == sub_words[0]:
            if all((i + j < total and srt_norm[i + j] == sw) for j, sw in enumerate(sub_words)):
                return i, "full"
    for i in range(search_from, total):
        if srt_norm[i] == sub_words[0]:
            return i, "partial"
...
def merge_srt(srt_content, text_content):
    ...
    if not entries: return None, None, "Could not parse SRT file"
    if not target_subs: return None, None, "Could not parse text file"
    ...
    for sub_idx, sub_text in enumerate(target_subs, 1):
        ...
        if srt_ptr >= total_words:
            log.append({"type": "SKIP", "idx": sub_idx, "text": sub_text[:55]})
            continue
        ...
        if start_ptr is None:
            ...
            log.append({"type": "MISSING", "idx": sub_idx, "ts": all_words[anchor]["start"], "text": sub_text[:55]})
            result.append({"text": sub_text, "start": all_words[anchor]["start"], "end": all_words[end_ptr]["end"]})
            continue
        srt_ptr = start_ptr + len(sub_words)
        end_ptr = min(srt_ptr - 1, total_words - 1)
        if match_type == "partial":
            log.append({"type": "APPROX", "idx": sub_idx, "ts": all_words[start_ptr]["start"], "reason": "first-word match only", "text": sub_text[:45]})
        elif start_ptr in imprecise_start or end_ptr in imprecise_end:
            reasons = []
            if start_ptr in imprecise_start: reasons.append("start")
            if end_ptr in imprecise_end: reasons.append("end")
            log.append({"type": "APPROX", "idx": sub_idx, "ts": all_words[start_ptr]["start"], "reason": f"boundary estimated ({', '.join(reasons)})", "text": sub_text[:40]})
        result.append({"text": sub_text, "start": all_words[start_ptr]["start"], "end": all_words[end_ptr]["end"]})

    if not result: return None, None, "Could not build any subtitles"
    result = fix_overlaps(result)
    lines = []
    for i, r in enumerate(result, 1):
        lines.append(f"{i}\n{r['start']} --> {r['end']}\n{r['text']}\n")
    output = "\n".join(lines)
    return output, log, f"{len(result)} subtitles"
...
def api_format():
    ...
    return jsonify({"result": format_text(data["text"])})
...
def api_merge():
    ...
    file_b64 = base64.b64encode(output.encode("utf-8")).decode("ascii")
